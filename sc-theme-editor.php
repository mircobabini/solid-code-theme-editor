...
function scte_theme_editor_page(){
		...
		if(preg_match('/^([\w\-]*)\.([\w\-]*)$/',$_POST['newfilename'],$filematches)){
			if(in_array($filematches[2],$allowedFileExt)){
				...
				if(!file_exists($newfile)){
					...
					echo '<script type="text/javascript">
						<!--
						window.location = \'themes.php?page=scte-theme-editor&theme=' . urlencode($theme->template) . '&file=' . urlencode(str_ireplace(end(explode("/",$file)),"",$file) . $_POST['newfilename']) . '&created=1\';
						//-->
						</script>';
					exit();
				}else{
	...
	if(isset($_POST['newcontent'])){
		...
		if (is_writeable($theme->get_stylesheet_directory() . '/' . $file)) {
			...
			if ($f !== FALSE) {
				...
				echo '<script type="text/javascript">
					<!--
					window.location = \'themes.php?page=scte-theme-editor&theme=' . urlencode($theme->template) . '&file=' . urlencode($file) . '&saved=1\';
					//-->
					</script>';
				exit();
			}
	...
	?>
					...
					<select name="theme" id="theme1">
						...
						foreach($themes as $stheme){
							if($theme->Name==$stheme['Name']){
								echo '<option selected="selected" value="'. $stheme->template .'">'. $stheme['Name'] .'</option>';
							}else{
								echo '<option value="'. $stheme->template .'">'. $stheme['Name'] .'</option>';	
							}
						}
						?>
					</select>
		...
		<div class="scte_content_left">
			<ul>
				<li style="float:left;"><a href="<?php echo (is_multisite() ? '': $pluginURL); ?>/solid-code-theme-editor/downloadfile/?theme=<?php echo urlencode($theme->template); ?>&amp;file=<?php echo urlencode($urlFile); ?>">Download File</a></li>
				<li style="float:left;margin-left:20px;"><a href="<?php echo (is_multisite() ? '': $pluginURL); ?>/solid-code-theme-editor/downloadbackup/?theme=<?php echo urlencode($theme->template); ?>">Download Whole Theme</a> (ZIP)</li>
			</ul>
			<div style="clear:both;"><!-- EMPTY --></div>
			<form id="newfile" name="newfile" method="post" action="themes.php?page=scte-theme-editor&amp;theme=<?php echo urlencode($theme->template); ?>&amp;file=<?php echo urlencode($urlFile); ?>">
			...
			<div style="clear:both;"><!-- EMPTY --></div>
			<form name="textarea_form" id="textarea_form" method="post" action="themes.php?page=scte-theme-editor&amp;theme=<?php echo urlencode($theme->template); ?>&amp;file=<?php echo urlencode($urlFile); ?>">
				<?php if(in_array($fxt[count($fxt)-1],$allowedFileExt)){ ?>
					...
					<div id="editor" style="z-index:100;position: relative;width: 700px;height: 600px;"><?php echo $content; ?></div>
		...
		<div class="scte_content_right">
			<div class="scte_inside_right">
				...
				echo scte_loopThroughFiles($theme->get_stylesheet_directory(),$theme->get_stylesheet_directory(),$theme->template,$urlFile);
				?>
			</div>
		</div>
		<div style="clear:both;"><!-- empty --></div>
